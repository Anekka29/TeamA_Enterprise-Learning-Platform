...
export default function Analytics({ dashboard, enrollments = [] }) {
  ...
  const activeCourses = data?.activeEnrolledCourses ?? 0;
  ...
  const weeklyPct = data?.weeklyProgressPercentage ?? 0;
  const monthlyPct = data?.monthlyProgressPercentage ?? 0;
  const quizPending = data?.quizzesPendingCount ?? 0;
  const achievements = data?.achievementsCount ?? 0;
