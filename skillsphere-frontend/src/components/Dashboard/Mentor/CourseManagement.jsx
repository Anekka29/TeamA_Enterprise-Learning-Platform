      ...
      <div className="study-window bg-white rounded-4 p-4 text-start shadow-lg" style={{ maxWidth: '720px', width: '92%', maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <div className="row g-4">
          <div className="col-lg-5">
            <div
              className="rounded-4 border d-flex align-items-center justify-content-center text-success fw-bold"
              style={{ minHeight: '220px', ...getCourseThumbnailStyle(course.thumbnailUrl) }}
            >
              {!course.thumbnailUrl && (
                <div className="text-center">
                  <i className="bi bi-collection-play-fill fs-1 d-block mb-2"></i>
...
export default function CourseManagement({ onShowToast }) {
  ...
  const openCourseAnalytics = (course) => {
    window.location.hash = '#mentor-analytics';
    onShowToast('info', `Metrics for "${course.title}" are available in the Metrics tab.`);
  };
  ...
  const renderCourseActions = (course) => {
    ...
    if (course.status === 'PUBLISHED') {
      actions.push(renderActionButton(course, 'View', 'bi-folder2-open', 'btn-primary', () => openCourseWizard(course, 'view'), 'open-view'));
      actions.push(renderActionButton(course, 'Analytics', 'bi-graph-up-arrow', 'btn-light border', () => openCourseAnalytics(course), 'analytics'));
    }
