...
export default function LandingPage() {
  ...
  // Compute Featured & Latest Courses dynamically
  ...
  // Rendering course card helper
  const renderCourseCard = (course) => {
    const hasImage = Boolean(course.thumbnailUrl || course.bannerUrl);
    const bannerStyle = hasImage
      ? { backgroundImage: `url(${course.thumbnailUrl || course.bannerUrl})`, backgroundSize: 'cover', backgroundPosition: 'center' }
      : { background: 'linear-gradient(135deg, #0d4a3a 0%, #064e3b 60%, #059669 100%)' };
    ...
    return (
      <div key={course.id} className="col-md-6 col-lg-4 reveal in-view">
        <div className="course-card-custom h-100 d-flex flex-column rounded-5 overflow-hidden">
          <div className="course-banner-img position-relative" style={{ height: '185px', ...bannerStyle }}>
            {!hasImage && (
              <div className="position-absolute inset-0 d-flex align-items-center justify-content-center w-100 h-100">
                <div className="text-center text-white px-3">
                  <i className="bi bi-mortarboard-fill display-5 opacity-75" style={{ color: '#34d399' }}></i>
                  <div className="fw-bold text-uppercase mt-1" style={{ fontSize: '0.68rem', letterSpacing: '0.15em', color: '#a7f3d0' }}>
                    SKILLSPHERE ACADEMY
                  </div>
                </div>
              </div>
            )}
            <span className="course-badge-category badge position-absolute top-0 start-0 m-3">{course.category || 'Tech'}</span>
            <span className="course-badge-level badge position-absolute top-0 end-0 m-3">{course.level || 'All Levels'}</span>
          </div>
  ...
  return (
    <LandingLayout>
      ...
      {featuredCourses.length > 0 && (
        <section className="section-pad bg-surface page position-relative" id="featured-courses">
          {/* Ambient Glass Glow Orbs */}
          <div className="ambient-glass-orb" style={{ width: '320px', height: '320px', background: 'radial-gradient(circle, rgba(16,185,129,0.18) 0%, transparent 70%)', top: '10%', left: '5%' }} />
          <div className="ambient-glass-orb" style={{ width: '280px', height: '280px', background: 'radial-gradient(circle, rgba(251,191,36,0.15) 0%, transparent 70%)', bottom: '10%', right: '5%', animationDelay: '-4s' }} />
          ...
          <div className="container container-xl position-relative z-1">
            <div className="text-center mx-auto reveal mb-5" style={{ maxWidth: 640 }}>
              <span className="section-eyebrow"><i className="bi bi-star-fill text-warning me-1"></i>Top Rated</span>
              <h2 className="section-title">Featured Courses</h2>
              <p className="section-desc mx-auto">Explore high-impact courses with top enrollment and review metrics.</p>
            </div>
      ...
      {latestCourses.length > 0 && (
        <section className="section-pad page position-relative" id="latest-courses">
          ...
          <div className="container container-xl position-relative z-1">
            ...
            <div className="row g-4 mt-2">
              {latestCourses.map((c) => renderCourseCard(c))}
            </div>
          </div>
        </section>
      )}
            ...
            <div className="row g-3 align-items-center">
              ...
              <div className="col-lg-6">
                <div className="d-flex flex-wrap gap-2 justify-content-lg-end">
                  {Array.from(new Set(['', 'Tech', 'Design', 'Business', ...courses.map((c) => c.category).filter(Boolean)])).map((cat) => (
                    <button
                      key={cat}
                      type="button"
          ...
          {loading ? (
            ...
            <>
              <div className="row g-4">
                {currentCourses.map((c) => renderCourseCard(c))}
              </div>
