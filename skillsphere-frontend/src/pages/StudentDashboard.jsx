...
export default function StudentDashboard() {
  ...
  // ─── Derived Data ────────────────────────────────────────────────────────────
  const activeCount = dashboard?.activeEnrolledCourses ?? 0;
  const completedCount = dashboard?.completedCourses ?? 0;
  ...
  const achievementsCount = dashboard?.achievementsCount ?? 0;
  const certificatesCount = dashboard?.certificatesCount ?? 0;
  ...
  const dashboardAchievements = dashboard?.recentAchievements || [];
      ...
      >
        ...
        {activeTab === '#dashboard' && (
          <div className="fade-in-quick">
            {/* Welcome hero */}
            <div className="welcome-card text-start">
              <div className="d-flex justify-content-between align-items-center">
                <div>
                  <h1 className="fw-bold mb-2" style={{ fontSize: '1.75rem' }}>
                    Welcome back, {dashboard?.studentName || profile?.fullName || user?.username || 'Student'}! 👋
                  </h1>
                  <p className="mb-0 text-white-50 fs-6">
                    You're on a <strong className="text-white">{currentStreak}-day streak</strong> 🔥
                    {continueLearningCourse
                      ? ` • Resume "${continueLearningCourse.title}"`
                          ...
                          <div className="d-flex justify-content-between align-items-center gap-3 flex-wrap">
                            <div className="d-flex align-items-center gap-3">
                              <div className="rounded-3 d-flex align-items-center justify-content-center"
                                style={{ width: '44px', height: '44px', background: 'rgba(16,185,129,0.08)', flexShrink: 0 }}>
                                <i className="bi bi-journal-bookmark-fill text-success fs-5"></i>
                              </div>
                              <div>
              ...
              <div className="row g-4">
                {displayEnrolledCourses.map(course => (
                  <div key={course.id} className="col-md-6 col-lg-4">
                    ...
                    >
                      <div style={{ background: 'linear-gradient(135deg, #0d4a3a, #166534)', padding: '20px' }}>
                        <div className="d-flex justify-content-between align-items-start">
                          <i className="bi bi-journal-bookmark-fill text-white" style={{ fontSize: '2rem' }}></i>
                          <span className="badge rounded-pill" style={{
                            background: (course.progress || 0) >= 100 ? '#fcd34d' : 'rgba(255,255,255,0.2)',
