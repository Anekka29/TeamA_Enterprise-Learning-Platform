...
public class GeminiProvider implements AIProvider {
    ...
    public String generateResponse(List<String> conversationHistory, String userMessage) {
        ...
        log.info("AI Provider executing - Mode: {}, Key prefix: {}", 
                isOpenRouter ? "OpenRouter (" + openRouterModel + ")" : "Native Gemini",
    ...
    private String generateOpenRouterResponse(List<String> conversationHistory, String userMessage) {
        try {
            ...
            systemMsg.put("content", "You are SkillSphere AI Tutor, an expert, friendly educational assistant on the SkillSphere platform. Help students understand course concepts, answer programming questions, debugging help, and career guidance clearly with code examples where applicable.");
            ...
            headers.set("X-Title", "SkillSphere AI Tutor");

            HttpEntity<Map<String, Object>> entity = new HttpEntity<>(requestBody, headers);

            log.info("Sending OpenRouter request to URL: {}, Model: {}", targetUrl, openRouterModel);
            ResponseEntity<String> response = restTemplate.exchange(targetUrl, HttpMethod.POST, entity, String.class);

            log.info("OpenRouter Response Status Code: {}", response.getStatusCode());
            ...
            if (choices.isArray() && choices.size() > 0) {
                ...
                if (!text.isBlank()) {
                    log.info("Successfully generated OpenRouter AI response (length: {})", text.length());
                    return text;
                }
            }

            log.error("No choices or empty text in OpenRouter response: {}", response.getBody());
            throw new AIServiceException("OpenRouter returned an empty response");

        } catch (HttpClientErrorException e) {
            log.error("OpenRouter Client Error - Status: {}, Response Body: {}", e.getStatusCode(), e.getResponseBodyAsString(), e);
            throw new AIServiceException("AI Service Error (" + e.getStatusCode() + "): " + e.getResponseBodyAsString());
        } catch (HttpServerErrorException e) {
            log.error("OpenRouter Server Error - Status: {}, Response Body: {}", e.getStatusCode(), e.getResponseBodyAsString(), e);
            throw new AIServiceException("AI Provider unavailable (" + e.getStatusCode() + ")");
        } catch (ResourceAccessException e) {
            log.error("OpenRouter Connection Error", e);
            throw new AIServiceException("AI service connection error: " + e.getMessage());
        } catch (Exception e) {
            log.error("Unexpected error in OpenRouter call", e);
