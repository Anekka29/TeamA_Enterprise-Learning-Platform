...
export default function MentorInternships({ onShowToast }) {
  const [activeTab, setActiveTab] = useState('postings'); // 'postings' | 'review' | 'create'
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [internshipsRes, appsRes] = await Promise.allSettled([
        InternshipService.getAllActiveInternships(),
        InternshipService.getMentorApplications(),
      ]);

      if (internshipsRes.status === 'fulfilled' && Array.isArray(internshipsRes.value.data)) {
        setPostedInternships(internshipsRes.value.data);
      }
  ...
  const handleCreateInternship = async (e) => {
    ...
    try {
      ...
      setPostedInternships(prev => [createdRes.data, ...prev]);
  ...
  const filteredApplications = filterJobId
    ? applications.filter(a => a.internship?.id === filterJobId)
    : applications;
    ...
    <div className="fade-in-quick text-start">
      <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
        <div>
          ...
          <p className="text-muted mb-0">Manage active internship postings, review candidate ATS dossiers, and accept applicants.</p>
        </div>

        <div className="bg-light p-1 rounded-pill border d-flex gap-1 flex-wrap">
          <button
            className={`btn btn-sm rounded-pill px-4 fw-bold ${activeTab === 'postings' ? 'btn-success text-white shadow-sm' : 'btn-light text-muted'}`}
            onClick={() => { setActiveTab('postings'); setFilterJobId(null); }}
          >
            <i className="bi bi-briefcase-fill me-1"></i> Posted Openings ({postedInternships.length})
          </button>
          <button
      ...
      {/* TAB 1: POSTED INTERNSHIPS GRID */}
      {activeTab === 'postings' && (
        <div className="mb-5">
          <div className="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
            <h5 className="fw-bold text-dark mb-0 d-flex align-items-center gap-2">
              <i className="bi bi-building-check text-success fs-4"></i>
              Active Partner & Mentor Internship Postings
            </h5>
            <span className="badge bg-success-subtle text-success border border-success-subtle px-3 py-2 rounded-pill fw-bold">
              {postedInternships.length} Openings Active
            </span>
          </div>
          ...
          {loading ? (
            <div className="text-center py-5 text-muted">
              ...
              <p className="fw-semibold">Loading posted internships...</p>
            </div>
          ) : postedInternships.length === 0 ? (
            <div className="text-center py-5 text-muted bg-white rounded-4 border">
              <i className="bi bi-briefcase fs-1 text-muted mb-2"></i>
              <p className="fw-bold mb-1">No internships posted yet.</p>
              <button className="btn btn-success btn-sm rounded-pill fw-bold px-4 mt-2" onClick={() => setActiveTab('create')}>
                <i className="bi bi-plus-circle me-1"></i> Post First Internship
              </button>
            </div>
          ) : (
            <div className="row g-4">
              {postedInternships.map(job => {
                const jobAppCount = applications.filter(a => a.internship?.id === job.id).length;
                    ...
                    <div className="card border-0 shadow-sm rounded-4 p-4 h-100 bg-white border hover-lift">
                      <div className="d-flex justify-content-between align-items-start mb-3">
                        <div className="d-flex gap-3 align-items-center">
                          <div className="rounded-4 bg-success-subtle text-success p-3 d-flex align-items-center justify-content-center fw-bold fs-3" style={{ width: '54px', height: '54px' }}>
                            {job.company ? job.company.charAt(0).toUpperCase() : 'C'}
                          </div>
                          <div>
                            <h5 className="fw-bold text-dark mb-0">{job.title}</h5>
                            <small className="text-muted fw-semibold">{job.company} • {job.postedByName || 'Mentor'}</small>
                          </div>
                        </div>
                        <span className="badge bg-success-subtle text-success border border-success-subtle rounded-pill px-3 py-2 fw-bold">
                      ...
                      <div className="d-flex align-items-center justify-content-between pt-3 border-top text-muted small flex-wrap gap-2">
                        <div className="d-flex gap-3">
                          ...
                          <span><i className="bi bi-cash-stack text-success me-1"></i>₹{job.stipendMin ? job.stipendMin.toLocaleString() : '20,000'} - ₹{job.stipendMax ? job.stipendMax.toLocaleString() : '30,000'} / mo</span>
                        </div>
                        <button
