...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      login(data.token, { email: data.email, role: actualRole, name: data.name || email.split('@')[0] }, rememberMe);
      ...
      const message = error?.message || error?.response?.data?.message || 'An error occurred. Please try again.';
