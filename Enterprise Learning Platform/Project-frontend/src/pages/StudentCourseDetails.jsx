...
export default function StudentCourseDetails() {
  ...
  const autoPlayRequested = location.state?.autoPlay || searchParams.get('play') === 'true';
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (courseData && courseData.enrolled && autoPlayRequested) {
        setShowPlayer(true);
      }
      ...
      if (intendedCourse && String(intendedCourse) === String(courseId) && isAuthenticated && user?.role === 'STUDENT') {
        ...
        if (courseData && !courseData.enrolled) {
          try {
            ...
            const updated = await loadCourseDetails();
            if (updated && updated.enrolled && autoPlayRequested) {
              setShowPlayer(true);
            }
          } catch (e) {
    ...
    init();
  }, [courseId, isAuthenticated, user, autoPlayRequested]);
    ...
    return (
      <div className="dashboard-wrapper-sim">
        <DashboardLayout sidebarLinks={sidebarLinks} searchPlaceholder="Search courses, roadmaps, mentors...">
          <div className="card border-0 shadow-sm rounded-4 p-5 text-center my-4 bg-white" style={{ minHeight: '55vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justify: 'center' }}>
            <i className="bi bi-exclamation-triangle-fill text-warning fs-1 mb-3"></i>
            <h3 className="fw-bold text-dark mb-2">Course Not Found</h3>
            <p className="text-muted mb-4" style={{ maxWidth: '480px' }}>
              The course you are trying to access (ID: {courseId}) was not found or is no longer available.
            </p>
            <div className="d-flex gap-3 justify-content-center flex-wrap">
              ...
              >
                <i className="bi bi-speedometer2 me-2"></i>Go to Student Dashboard
              </button>
              <button
                className="btn btn-outline-secondary rounded-pill px-4 fw-bold"
                onClick={() => navigate('/student-dashboard#learning')}
              >
                <i className="bi bi-grid me-2"></i>Browse Catalog
              </button>
            </div>
          </div>
        </DashboardLayout>
