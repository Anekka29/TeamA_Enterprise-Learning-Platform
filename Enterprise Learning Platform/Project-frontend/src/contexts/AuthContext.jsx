...
export function AuthProvider({ children }) {
  ...
  // Initialise from tab-scoped storage on mount & refresh from API
  useEffect(() => {
    const storedUser = AuthUtils.getCurrentUser();
  ...
  // Listen for profile update events within current component tree
  useEffect(() => {
    const handleUpdate = () => {
      refreshUser();
    };
  ...
  const login = useCallback((token, userData, rememberMe = false) => {
