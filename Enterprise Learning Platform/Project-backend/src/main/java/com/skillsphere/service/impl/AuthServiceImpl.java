    ...
    public LoginResponse login(LoginRequest request) {
        ...
        if (user.getPassword() != null) {
            try {
                ...
                log.warn("BCrypt match failed for user {}: {}", user.getEmail(), e.getMessage());
            }
        ...
        if (request.getSelectedRole() != null && request.getSelectedRole() != user.getRole()) {
            log.warn("Role mismatch for user {}: selected role {}, but account role is {}", 
                    user.getEmail(), request.getSelectedRole(), user.getRole());
            throw new RoleMismatchException("The selected role (" + request.getSelectedRole() + ") does not match this account's role. Please select the correct role to log in.");
        }
