...
public class AdminController {
    ...
    public ResponseEntity<Map<String, Object>> getAnalyticsDetails() {
        ...
        // Real Telemetry Data (Server CPU, Memory, Uptime, Threads)
        ...
        try {
            ...
            log.warn("Could not read OS CPU bean: {}", e.getMessage());
        }

        if (cpuLoad < 0 || Double.isNaN(cpuLoad)) {
            cpuLoad = Math.min(0.85, (double) activeThreads / (availableProcessors * 20.0));
        }

        int cpuPercent = Math.max(1, (int) Math.round(cpuLoad * 100));

        telemetry.put("cpuPercent", cpuPercent);
        telemetry.put("usedMemoryMb", usedMemoryMb);
        telemetry.put("totalMemoryMb", totalMemoryMb);
        telemetry.put("maxMemoryMb", maxMemoryMb);
        telemetry.put("memoryUsagePercent", Math.round(memoryUsagePercent));
        telemetry.put("uptimeMinutes", uptimeMinutes);
        telemetry.put("activeThreads", activeThreads);
        telemetry.put("availableProcessors", availableProcessors);
        ...
        data.put("draftCourses", draftCourses);
        ...
        data.put("archivedCourses", archivedCourses);
